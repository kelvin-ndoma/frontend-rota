...
import { useLocation } from 'react-router-dom'; // Import useLocation to access state from previous page

function Mypayments() {
  const location = useLocation(); // Get the location object to access state from the previous page
  ...
  // Extract event data from location state
  const eventData = location.state ? location.state.event : null;
  ...
  return (
    <div className="App bg-slate-700 h-screen text-white w-full">
      <header className="App-header">
        ...
        {eventData && <p>Payment for: {eventData.name}</p>}
      </header>
